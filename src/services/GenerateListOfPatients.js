...
class GenerateListOfPatients {
	...
	execute(
		...
		if (littleBehaviorFraction > 1) {
			throw new Error("The parameter 'littleBehaviorFraction' must be lesser than 1.");
		}

		if (topListLength < 1) {
			throw new Error("The parameter 'topListLength' must be greater than zero.");
		}

		if (littleBehaviorScoreThreshold < 1) {
			throw new Error("The parameter 'littleBehaviorScoreThreshold' must be greater than zero.");
		}
		...
		const topList = sortedPatients.slice(0, topListLength - littleBehaviorPatientsToAdd);
		const restOfPatients = sortedPatients.slice(
			topListLength - littleBehaviorPatientsToAdd,
			patients.length
		);

		const sortedByBehavior = restOfPatients.sort((p1, p2) => p1.behaviorScore - p2.behaviorScore);
		...
		for (let i = 0; i < littleBehaviorPatientsToAdd; i += 1) {
			const randomIndex = Math.floor(Math.random() * littleBehaviorPatients.length);
...
module.exports = GenerateListOfPatients;
